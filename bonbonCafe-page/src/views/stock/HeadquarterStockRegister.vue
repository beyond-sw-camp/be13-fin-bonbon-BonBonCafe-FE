<template>
  <div>
    ...
    <v-form @submit.prevent="submitStock">
      <!-- ✅ 재료 선택 -->
      <v-select v-model="form.ingredientId" :items="ingredientOptions" item-title="ingredientName"
        item-value="ingredientId" label="재료 선택" required />

      <!-- ✅ 수량 입력 -->
      <v-text-field v-model="form.quantity" label="수량" type="number" required />
