              ...
              <v-row dense>
                ...
                <v-col cols="10" offset-md="1">
                  ...
                  <v-dialog v-model="dialogVisible" max-width="600px" eager>
                    <v-card>
                      <v-card-title class="text-h6">지역 선택</v-card-title>
                      <v-card-text>
                        <v-text-field
...
const isDirty = ref(false);
  ...
  const fetchRegionList = async (page, size, search = '') => {
    try {
  ...
  watch(selectedHeadquarter, (newVal) => {
    if (newVal && newVal.headquarterId) {
      ...
      fetchRegionList(dialogCurrentPage.value, dialogPageSize.value, dialogSearch.value, newVal.headquarterId);
    } else {
      editedInfo.value.headquarterId = null;
      // 헤드쿼터 선택 해제 시 전체 리스트 불러오기
      dialogCurrentPage.value = 1;
      fetchRegionList(dialogCurrentPage.value, dialogPageSize.value, dialogSearch.value, null);
    }
  });
  ...
  const selectRegion = (item) => {
    ...
    console.log("selected Region : ", editedInfo.value);
    isDirty.value = true;
    dialogVisible.value = false;

    alert(`"${editedInfo.value.region}" 지역을 선택했습니다.`);
  };

  watch(dialogVisible, async (visible) => {
    if (visible) {
      await nextTick(); // DOM 완전히 그린 후
      fetchRegionList(dialogCurrentPage.value, dialogPageSize.value, dialogSearch.value, editedInfo.value.headquarterId);
    }
  });
...
// 검색어 입력 시 첫 페이지부터 다시 검색된 목록 보여줌
  watch(dialogSearch, () => {
    dialogCurrentPage.value = 1;
    fetchRegionList(1, dialogPageSize.value, dialogSearch.value, editedInfo.value.headquarterId);
  });

  // 페이지네이션에서 페이지 변경 시 필요
  const onDialogPageChange = (page) => {
    dialogCurrentPage.value = page;
    fetchRegionList(page, dialogPageSize.value, dialogSearch.value, editedInfo.value.headquarterId);
  };
  ...
  const onDialogPageSizeChange = (size) => {
    ...
    dialogCurrentPage.value = 1;
    fetchRegionList(1, size, dialogSearch.value, editedInfo.value.headquarterId);
  };
  ...
  const onDialogOptionsChange = (options) => {
    ...
    fetchRegionList(options.page, options.itemsPerPage, dialogSearch.value, editedInfo.value.headquarterId);
  };


  const deleteRegion = () => {
    if (!editedInfo.value.regionCode) return;

    const confirmed = confirm('현재 등록된 가맹점을 삭제하시겠습니까?');
    if (!confirmed) return;

    editedInfo.value.regionCode = null;
    editedInfo.value.region = '';
    isDirty.value = true;
  ...
  onMounted(() => {
    fetchHeadquarters();
    fetchRegionList(currentPage.value, pageSize.value, search.value, null);
  });
