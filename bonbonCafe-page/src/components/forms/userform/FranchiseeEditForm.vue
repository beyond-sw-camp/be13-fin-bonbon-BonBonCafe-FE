<template>
    <v-container class="py-4" fluid>
      <div class="mb-4 text-subtitle-1 font-weight-medium">
        계정 관리 /
        <span class="text-primary font-weight-bold">가맹점주 정보 수정</span>
      </div>
  
      <v-row dense>
        <!-- 수정 카드 -->
        <v-col cols="12" md="6">
          <v-card class="pa-6" elevation="2" style="width: 100%; height: 720px;">
  ...
  const statusOptions = ['ACTIVE', 'INACTIVE', 'PENDING', 'EXPIRED', 'DELETED'];
...
const onFileChange = (file) => {
  ...
  if (Array.isArray(file)) {
    ...
    // v-file-input에서 자동으로 설정된 값이 있는 경우 fallback
    selectedFile = profileFile.value instanceof File ? profileFile.value : null;
  }
  ...
  if (selectedFile instanceof File) {
    ...
    console.warn('선택된 파일이 유효한 File 객체가 아닙니다:', file);
    previewImage.value = null;
    profileFile.value = null;
  }
};
  ...
  const submitEdit = async () => {
    try {
      ...
      await apiClient.post(
        `/bonbon/user/franchisee-accounts/${franchiseeId}`,
        editedInfo.value,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
