 <!-- <template>
    <form @submit.prevent="handleSubmit">
        <v-table class="pa-0 d-flex">
        <tbody>
          <tr>
            <td class=" thead-title tb-bg"><span class="required">* </span> 가맹점 이름</td>
            <td class=""><input type="text"/></td>
            <td class=" thead-title"><span class="required">* </span> 점주 이름</td>
            <td class=""><input type="text"/></td>
          </tr>
          <tr>
            <td class=" thead-title "><span class="required">* </span> 가맹점 사진</td>
              <td colspan="1">
                <div class="ma-5 upload-box">사진 올리기</div>
                <input type="file" class="file-input" />
              </td>
            <td class=" thead-title"><span class="required">* </span> 점주 사진</td>
            <td colspan="1">
                <div class="upload-box">사진 올리기</div>
                <input type="file" class="file-input" />
              </td>
          </tr>
          <tr>
            <td class=" thead-title"><span class="required">* </span> 가맹점 전화번호</td>
            <td class=""><input type="tel"/></td>
            <td class=" thead-title"><span class="required">* </span> 담당자</td>
            <td>
              <select>
                <option disabled selected>선택</option>
                <option>홍길동</option>
              </select>
            </td>
          </tr>
          <tr>
            <td class="thead-title" rowspan="2"><span class="required">* </span>우편 번호</td>
            <td rowspan="2">
              <input type="text" />
              <button class="search-btn">🔍</button>
            </td>
            <td class="thead-title">주차 가능</td>
            <td>
              <label><input type="checkbox" /> 가능</label>
              <label><input type="checkbox" /> 불가</label>
            </td>
          </tr>
          <tr>
            <td class=" thead-title"><span class="required">* </span> 운영상태</td>
            <td class="">ffff</td>
          </tr>
          <tr>
            
            <td class=" thead-title"><span class="required">* </span> 운영 시간</td>
            <td class="">ddd</td>
          </tr>
          <tr>
            <td class=" thead-title"><span class="required">* </span> 개업 일자 </td>
            <td>
              <select> <option >년</option> </select>
              <select> <option >월</option> </select>
              <select> <option >일</option> </select>
            </td>
            <td class=" thead-title"> 좌석수</td>
            <td><input type="number" /></td>
          </tr>
        </tbody>
        </v-table>
    </form>
</template>


<script setup>

</script>
<style scoped>
.thead-title{
  width: 120px; 
  max-width: 150px;
  white-space: nowrap;
  overflow: hidden;
  text-overflow: ellipsis;
  background-color: #efefef;
}

.franchise-register {
  background: #fdfcf6;
  padding: 30px;
  font-family: 'Pretendard', sans-serif;
}

.title {
  font-size: 24px;
  font-weight: 800;
  margin-bottom: 20px;
  color: #1a1a1a;
}

.register-table {
  width: 100%;
  border-collapse: separate;
  border-spacing: 0 10px;
}

.register-table td {
  padding: 10px;
  vertical-align: middle;
}

.register-table input,
.register-table select {
  padding: 6px 10px;
  width: 100%;
  box-sizing: border-box;
  border: 1px solid #ccc;
  border-radius: 8px;
}

.upload-box {
  height: 200px;
  border: 2px dashed #ccc;
  text-align: center;
  padding: 20px;
  color: #aaa;
  background-color: #efefef;
  border-radius: 12px;
  cursor: pointer;
}

.required {
  color: red;
}

.search-btn {
  margin-left: 4px;
  background: #f1f1f1;
  border: none;
  padding: 6px 10px;
  border-radius: 6px;
  cursor: pointer;
}

.btn-wrap {
  margin-top: 30px;
  text-align: center;
}

.btn {
  padding: 10px 30px;
  margin: 0 10px;
  font-weight: 600;
  border-radius: 10px;
  border: none;
  cursor: pointer;
}

.submit {
  background-color: #b59f78;
  color: white;
}

.cancel {
  background-color: #f1f1f1;
  color: #333;
}

</style> -->


<template>
  <div class="franchise-register">
    <!-- <h2 class="title">가맹점 등록</h2> -->

    <v-table class="register-table">
      <tbody>
        <tr>
          <td class="thead-title"><span class="required">* </span>가맹점 이름</td>
          <td><input type="text" /></td>
          <td class="thead-title"><span class="required">* </span>점주 이름</td>
          <td><input type="text" /></td>
        </tr>

        <tr>
          <td class="thead-title"><span class="required">* </span>매장 사진</td>
          <td>
            <div class="upload-box">사진 올리기</div>
            <input type="file" class="file-input" />
          </td>
          <td class="thead-title"><span class="required">* </span>점주 사진</td>
          <td>
            <div class="upload-box">사진 올리기</div>
            <input type="file" class="file-input" />
          </td>
        </tr>

        <tr>
          <td class="thead-title"><span class="required">* </span>매장 전화번호</td>
          <td><input type="text" /></td>
          <td class="thead-title"><span class="required">* </span>담당자</td>
          <td>
            <select>
              <option disabled selected>선택</option>
              <option>홍길동</option>
            </select>
          </td>
        </tr>

        <tr>
          <td class="thead-title"><span class="required">* </span>우편 번호
        </td>
          <td>
            <input type="text" />
            <button class="search-btn">🔍</button>
          </td>
          <td class="thead-title">주차 가능</td>
          <td>
            <label><input type="checkbox" /> 가능</label>
            <label><input type="checkbox" /> 불가</label>
          </td>
        </tr>

        <tr>
          <td class="thead-title">상세 주소</td>
          <td colspan="3"><input type="text" style="width: 100%;" /></td>
        </tr>

        <!-- 사업자번호 제거됨 -->

        <tr>
          <td class="thead-title">개업 일자</td>
          <td colspan="">
            <v-select> <option>년</option> </v-select>
            <v-select> <option>월</option> </v-select>
            <v-select> <option>일</option> </v-select>
          </td>
           <td class="thead-title">운영 상태</td>
            <td>
              <label><input type="checkbox" /> 운영중</label>
              <label><input type="checkbox" /> 운영 중단</label>
            </td>
        </tr>

        <tr>
          <td class="thead-title">좌석수</td>
          <td><input type="number" /></td>
          <td class="thead-title">운영 시간</td>
          <td>
            <input type="time" /> ~ <input type="time" />
          </td>
        </tr>

      </tbody>
    </v-table>

  </div>
</template>

<style scoped>
.franchise-register {
  background-color: white;
  /* padding: 0px; */
  /* margin: 0%; */
  /* font-family: 'Pretendard', sans-serif; */
}

.title {
  font-size: 24px;
  font-weight: 800;
  margin-bottom: 20px;
  color: #1a1a1a;
}

.thead-title{
  width: 120px; 
  max-width: 150px;
  white-space: nowrap;
  overflow: hidden;
  text-overflow: ellipsis;
  background-color: #efefef;
}
.register-table {
  width: 100%;
  border-collapse: separate;
  /* border-spacing: 0 10px; */
}

.register-table td {
  padding: 10px;
  vertical-align: middle;
}

.register-table input,
.register-table select {
  padding: 6px 10px;
  width: 100%;
  box-sizing: border-box;
  border: 1px solid #ccc;
  border-radius: 8px;
}

.upload-box {
  border: 2px dashed #ccc;
  text-align: center;
  padding: 20px;
  color: #aaa;
  border-radius: 12px;
  cursor: pointer;
}

.required {
  color: red;
}

.search-btn {
  margin-left: 4px;
  background: #f1f1f1;
  border: none;
  padding: 6px 10px;
  border-radius: 6px;
  cursor: pointer;
}

.btn-wrap {
  margin-top: 30px;
  text-align: center;
}

.btn {
  padding: 10px 30px;
  margin: 0 10px;
  font-weight: 600;
  border-radius: 10px;
  border: none;
  cursor: pointer;
}

/* .submit {
  background-color: #b59f78;
  color: white;
}

.cancel {
  background-color: #f1f1f1;
  color: #333;
} */
</style>
